...
fig = go.Figure()
df = pd.read_csv("model_generation/comparing_models/csv_files/M3.csv")
...
new_df.to_csv("model_generation/comparing_models/csv_files/M3_resampled.csv", index=False)

df = pd.read_csv("model_generation/comparing_models/csv_files/M3_resampled.csv")
vals = np.zeros((len(df), 7))
vals[:, 0] = df["x"]      
vals[:, 1] = df["y"]    
vals[:, 2] = df["z"]       

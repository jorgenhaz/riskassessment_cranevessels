...
# State-vectors
q = Matrix([x_n, y_n, z_n, phi, theta, psi, q1_p, q2_p])
omega = Matrix([uq, vq, wq, pq, qq, rq, q1_pdot, q2_pdot])

# External forces
tau_xn, tau_yn, tau_zn, tau_phi, tau_theta, tau_psi, tau_q1p, tau_q2p = symbols('tau_xn tau_yn tau_zn tau_phi tau_theta tau_psi tau_q1p tau_q2p')
...
all_vars = Matrix([x_n, y_n, z_n, phi, theta, psi, q1_p, q2_p,
                   uq, vq, wq, pq, qq, rq, q1_pdot, q2_pdot,
                   tau_xn, tau_yn, tau_zn, tau_phi, tau_theta, tau_psi, tau_q1p, tau_q2p,
...
betta = pad_matrix(betta, len(q), len(q))
betta[6,6] = 1
betta[7,7] = 1
betta_T = betta.T
alpha_T = (betta.inv())
...
transformList = [T_b_q4*rot_x(q1_p), T_b_payload]
rotAxList = [
    pm.UnitVectors.i,
    pm.UnitVectors.j
]
...
# Restoring CG
r_local_q1 = Matrix([0, 0, pm.Joint1Params.length/2, 1])
r_local_q2 = Matrix([-pm.Joint2Params.length/2, 0, 0, 1])
r_local_q3 = Matrix([-pm.Joint3Params.length/2, 0, 0, 1])
r_local_q4 = Matrix([-(q4+pm.Joint4Params.length)/2, 0, 0, 1])
r_q1_cg = Matrix((T_b_q1_cog * r_local_q1)[0:3])
r_q2_cg = Matrix((T_b_q2_cog * r_local_q2)[0:3])
r_q3_cg = Matrix((T_b_q3_cog * r_local_q3)[0:3])
r_q4_cg = Matrix((T_b_q4_cog * r_local_q4)[0:3])
...
r_boat_cog = (
    ...
    + pm.Joint1Params.mass * r_q1_cg
    + pm.Joint2Params.mass * r_q2_cg
    + pm.Joint3Params.mass * r_q3_cg
    + pm.Joint4Params.mass * r_q4_cg
) / mass_boat
...
I_q1 = parallel_axis(I_q1_local, pm.Joint1Params.mass, dr_boat(r_q1_cg))
I_q2 = parallel_axis(I_q2_local, pm.Joint2Params.mass, dr_boat(r_q2_cg))
I_q3 = parallel_axis(I_q3_local, pm.Joint2Params.mass, dr_boat(r_q3_cg))
I_q4 = parallel_axis(I_q4_local, pm.Joint2Params.mass, dr_boat(r_q4_cg))
...
r_payload_cg = Matrix(T_b_payload[0:3, 3])

r_cog_system = (mass_boat * r_boat_cog
                + m_payload * r_payload_cg)/m_tot
...
D[6,6] = 2*zeta_d*sqrt(m_payload * ((m_payload*g)/wire_length))
D[7,7] = 2*zeta_d*sqrt(m_payload * ((m_payload*g)/wire_length))
...
"""Generating C++-code"""
# Filepaths
filepath_cpp_wave = "../code/cpp_sim_testing/M5_wave/src"
filepath_h_wave   = "../code/cpp_sim_testing/M5_wave/include"
filepath_cpp = "../code/cpp_sim_testing/M5/src"
filepath_h   = "../code/cpp_sim_testing/M5/include"
...
# with open ("../code/cpp_sim_testing/M5_wave/files/init.yaml", "w") as f:
#     for var in all_vars:
#         f.write(f"{var}: 0.0 \n")
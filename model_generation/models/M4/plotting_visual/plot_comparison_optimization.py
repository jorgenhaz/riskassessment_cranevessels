...
def plot_multiple_named(var_names, df_euler, df_quasi):
    ...
    for i, var in enumerate(var_names):
        axes[i].plot(df_euler["time"], np.rad2deg(df_euler[var]) if 'phi' in var or 'theta' in var or 'psi' in var else df_euler[var], label='optimized')
        axes[i].plot(df_quasi["time"], np.rad2deg(df_quasi[var]) if 'phi' in var or 'theta' in var or 'psi' in var else df_quasi[var], label='not optimized')
...
df_euler = pd.read_csv("model_generation/models/model_1/optimized_with_euler.csv")
df_quasi = pd.read_csv("model_generation/models/model_1/not_optimized_euler.csv")
...
plt.plot(df_euler['time'], np.rad2deg(df_euler["q1"]), label='optimized')
plt.plot(df_quasi['time'], np.rad2deg(df_quasi["q1"]), label='not optimized')

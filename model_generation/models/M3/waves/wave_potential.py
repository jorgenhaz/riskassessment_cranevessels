...
from model_generation.models.M3.waves.wave_utils import jonswap_spectrum, dynamical_pressure_field, wave_accelerations
import yaml
...
with open('model_generation/models/M3/waves/wave_model_input.yaml', 'r') as file:
...
filepath_cpp = "../code/cpp_sim_testing/M3_wave/src"
filepath_h = "../code/cpp_sim_testing/M3_wave/include"
...
# omega, S_omega, a = jonswap_spectrum(Hs=Hs, Tp=Tp, gamma=Gamma, N=N, f_min=f_min, f_max=f_max)

# plt.figure(figsize=(10, 6))
# plt.plot(omega, S_omega, linewidth=1.8, label='JONSWAP spectrum')
# plt.grid(True)
# plt.xlim(omega.min(), omega.max())
# # plt.legend(fontsize=18, loc='upper right')

# plt.xlabel(r'Frequency $\omega$ (rad/s)', fontsize=18)
# plt.ylabel(r'Spectral density $S(\omega)$ (m$^2$s/rad)', fontsize=18)
# plt.title("JONSWAP Spectrum", fontsize=20, fontweight='bold', pad=40)
# plt.text(0.5, 1.02, rf"$H_s$ = {Hs} m, $T_p$ = {Tp} s, $\gamma$ = {Gamma}", 
#          fontsize=18, ha='center', va='bottom', transform=plt.gca().transAxes, fontweight='bold')

# plt.show()

# """Control check"""
# m0 = np.trapz(S_omega, omega)
# Hs_calc = 4 * np.sqrt(m0)
# print("Input Hs =", Hs, "→ Hs_calc =", Hs_calc)
...
Eigen::VectorXd model(Eigen::VectorXd kwargs) {
    ...
    Eigen::VectorXd dp    =   Beta_transpose * tau 
                              + C_A*omega
                              -Beta_transpose*gamma_T_diff_omega(kwargs)
                              +Beta_transpose*T_diff_q(kwargs)
                              -Beta_transpose*P_diff_q(kwargs)
                              -Beta_transpose * G_restoring
                              -Beta_transpose * D_q_dot(kwargs);
